...
- **Magic Link auth** — Optional sign-in via email for write access (read access is public)
- **Audit trail** — Every add/update/delete is logged with the user's email
...
If you want Magic Link sign-in for write access:

1. **Supabase Dashboard → Authentication → Providers → Email** → Enable
2. **Authentication → URL Configuration** → Add your Vercel URL as a Redirect URL
3. Insert approved emails into the `allowed_users` table:
   ```sql
...
- **Read (SELECT)**: Open to both `anon` and `authenticated` users (the dashboard is publicly readable)
- **Write (INSERT/UPDATE/DELETE)**: Restricted to `authenticated` users only
- **Audit log**: Authenticated users can SELECT and INSERT (no update/delete)
